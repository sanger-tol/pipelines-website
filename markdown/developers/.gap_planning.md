...
- Possibly v1.3.5 that introducs an option to skip PacBio filtering/trimming
...
- Release features already implemented on the development branch
  ...
  - Support for ULI
  - Better CRAM support
- (feature) Align long-read data via code shared with and reviewed by ToLA
  - Must be in v1.4.0
- (feature) `@SQ` lines in the CRAM headers
  - Should include aliases, species name, etc
  - One implementation already available based on the current assembly download pipeline.
  - _Might_ have to change depending on what the [new header](#insdcdownload-now) will have.
  - Severity / importance still being assessed
- (bug) Update CPU/memory settings
  ...
  - For v1.4.1
- (feature) samplesheet update to capture the specimen and run\_accession instead of the sample
  - For v1.4.0
...
- Support for PiMMS
  - For v1.5.0
- Confirm ONT preprocessing is correct
  - For v1.5.0
- Support for RNA-Seq
  - For v1.6.0
- Update the input parameter names and the output file names as per our new [standards]{#standards}.
...
- (feature) make the merging of the input read alignment optional
  - For variantcalling v1.2.0
  - Will make it work with the new specimen/run\_accession approach
...
- Work off specimen and run\_accession like the other pipelines
  - For v0.11.0
