...
- Extra output file that lists the sequences in the correct order for contact maps
  - Rules:
    - Chromosomes first, then unplaced sequences
    - Chromosomes ordered by number (supporting Arabic and Roman numerals naming schemes), falling back to decreasing size if the naming scheme is not recognised
      - Autosomes first and then sex chromosomes
      - Sex chromosomes identified by their name (W, X, Y, Z, U, V)
    - For each chromosome:
      - "assembled-molecule" sequences first, ordered by chunk number
      - all unlocalised sequences, by decreasing size
    - Unplaced sequences by decreasing size
  - will be passed to `samtools faidx` when Fasta reordering is needed
...
- (feature) remove features (except the stats) released in variantcomposition 0.2
...
### variantcalling 2.0 and variantcomposition 2.0
...
- Add comprehensive repeat finders such as EarlGreyTE or EDTA2
- Add TRASH and Pantera
- Add stainedglass and/or ModDotPlot for self-alignment
- Mappability tracks
- Compute gene annotation with tools such as BRAKER, EGAPx, or Helixer
